...
export function convertTools(options: vscode.LanguageModelChatRequestHandleOptions): {
	...
	if (options.toolMode === vscode.LanguageModelChatToolMode.Required) {
		if (tools.length !== 1) {
			console.error("[Synthetic Model Provider] ToolMode.Required but multiple tools:", tools.length);
...
export function validateTools(tools: readonly vscode.LanguageModelChatTool[]): void {
	for (const tool of tools) {
		if (!tool.name.match(/^[\w-]+$/)) {
			console.error("[Synthetic Model Provider] Invalid tool name detected:", tool.name);
			throw new Error(
...
export function validateRequest(messages: readonly vscode.LanguageModelChatRequestMessage[]): void {
	...
	if (!lastMessage) {
		console.error("[Synthetic Model Provider] No messages in request");
		...
		if (message.role === vscode.LanguageModelChatMessageRole.Assistant) {
			...
			while (toolCallIds.size > 0) {
				...
				if (!nextMessage || nextMessage.role !== vscode.LanguageModelChatMessageRole.User) {
					console.error("[Synthetic Model Provider] Validation failed: missing tool result for call IDs:", Array.from(toolCallIds));
					...
					if (!isToolResultPart(part)) {
						const ctorName =
							(Object.getPrototypeOf(part as object) as { constructor?: { name?: string } } | undefined)?.constructor
								?.name ?? typeof part;
						console.error("[Synthetic Model Provider] Validation failed: expected tool result part, got:", ctorName);
						throw new Error(errMsg);
					}
					const callId = (part as { callId: string }).callId;
